...
\todo{Short paragraph; find something else to talk about here.}
...
\todo{Choose appropriate subscripts and add to nomenclature.}
...
\todo{Look up and add $\text{Re}$ number correction.}
...
\end{equation}
\nomenclature[A$Re$]{$\RE_D$}{Reynolds number based on diameter, $\text{Re}_D=\rho \vel D/\mu$\nomunit{-}}
...
\nomenclature[Y]{$\Delta\left(\;\right)$}{Change in quantity, \eg{} $\text{final} - \text{initial or outlet} - \text{inlet}$\nomunit{$\left[\;\right]$}}
