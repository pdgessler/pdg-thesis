...
\end{align}
where $\dot{m}_{\rfr}$ and $\dot{m}_{\wat}$, respectively, are used to simplify the notation.
...
For a phase-change application such as the condensation process here, 
...
Finally, the energy balance for the refrigerant stream may be written as
\begin{align}
  0 &= -\dot{Q} + \dot{m}_{\rfr}(h_{\rfr,\inl} - h_{\rfr,\out}), \\
  \shortintertext{and, for the water stream,}
...
on an overall basis and not per tower.
The aggregated cooling tower has two flow streams: a water stream
and a psychrometric mixture of air and water vapor.
These flow streams are mixed and a portion of the water stream
evaporates into the air-water mixture.
By convention, the mass balance for the air-water vapor mixture
is written in terms of the dry air mass flow rates, so
\begin{equation}
  \dot{m}_{\air,\inl} = \dot{m}_{\air,\out} (=\dot{m}_{\air}).
\end{equation}
For the water stream, an additional term, $\dot{m}_{\wat,\text{evap}}$, accounts for
the water evaporated into the air-water vapor mixture stream,
...
air-water vapor mixture outlet and inlet, respectively.
...
\emph{saturation specific heat}, $c_s$, is approximated by
\begin{equation}
...
for a saturated psychrometric mixture.
...
a semi-empirical approach; this relationship is
\begin{equation}
...
\begin{equation}
  \dot{Q} = \epsilon_{\air} \dot{m}_{\air} (h_{\air,\sat,T=T_{\wat,\inl}} - h_{\air,\inl}),
\end{equation}
where the maximum possible heat transfer is calculated by assuming that the outlet
air is fully saturated at the inlet water temperature 
(the difference $h_{\air,\sat,T=T_{\wat,\inl}} - h_{\air,\inl}$).
...
The throttle is modeled as a rigid control volume and no shaft work ($\dot{W}=0$).
Additionally, it is well-insulated ($\dot{Q}=0$). The mass balance for this single input
single output system is
\begin{equation}
  \dot{m}_{\inl} = \dot{m}_{\out} (=\dot{m}).
\end{equation}
The energy balance then reduces to
\begin{equation}
  h_{\inl} = h_{\out}.
\end{equation}
The pressure at the exit of each throttle is assumed to be the suction pressure
of the current operating point, which allows the outlet states to be fixed.

\section{Mixing Chamber} \label{sec:Mixing}
The mixing chamber is assumed to be a rigid control volume which is also well-insulated.
It has two inputs (condensed and bypassed refrigerant) and one outlet leading to the
...
\end{equation}
The two balances for this component constrain the division of flow between
